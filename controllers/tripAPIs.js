...
router.post(
    "/add",
    VerifyToken,
    express_uploadFile({ createParentPath: true }),
    async(req, resp) => {
        debugger;
        var images = req.files.images;
        var ImagesIDs = [];
        const { name, description, days, nights, date } = req.body;
        debugger;
        ...
        new_Trip.save(async(err, data) => {
            debugger;
            if (err) {
                resp.send(
                    JSON.stringify({
                        status: false,
                        message: "failed: something error 1",
                        data: err.message,
                    })
                );
            } else {
                ...
                resp.send(
                    JSON.stringify({ status: true, message: "success", data: data })
                );
...
router.put("/update", VerifyToken, async(req, resp) => {
    const { name, description, days, nights, date, tripID, Images } = req.body;
    ...
    if (findTrip) {
        ...
        resp.send(
            JSON.stringify({ status: false, message: "failed: Trip not found" })
        );
    }
});
...
router.get("/getAllTrips", VerifyToken, (req, resp) => {
    TripModel.find({ date: { $gte: new Date() } },
            "name description days nights date images"
        )
...
router.get("/:TripId", VerifyToken, async(req, resp) => {
    const { TripId } = req.params;
    TripModel.findOne({ _id: TripId },
            "_id name description days nights date images"
        )
        .populate({ path: "Images", select: "path" })
        .exec((err, data) => {
            if (!err) {
